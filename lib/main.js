...
const rpcClientId = '1371491106981941410';
...
function flashVersion() {
    if (process.platform === 'darwin') return '34.0.0.372';
    if (process.platform === 'linux') return '34.0.0.137';
    return '34.0.0.330';
}
...
function startRpc() {
    ...
    try {
        ...
        rpc.login({ clientId: rpcClientId }).catch(stopRpc);
    } catch (_) {
        stopRpc();
    }
}
...
async function createWindow() {
    ...
    buildMenu();
    mainWindow.webContents.setUserAgent(`Moshi Online Client v${app.getVersion()}`);
